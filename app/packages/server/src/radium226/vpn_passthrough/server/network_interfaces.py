...
class NetworkInterfaces:
    def __init__(self, name: str, veth: str, vpeer: str, veth_addr: str, vpeer_addr: str, prefix_len: int) -> None:
        self._name = name
        self._veth = veth
        self._vpeer = vpeer
    ...
    def subnet(self) -> str:
        ...
        return str(ipaddress.IPv4Interface(f"{self._veth_addr}/{self._prefix_len}").network)

    @staticmethod
    @asynccontextmanager
    async def add(namespace: Namespace, veth_addr: str | None = None, vpeer_addr: str | None = None, cidr: str | None = None) -> AsyncIterator["NetworkInterfaces"]:
        name = namespace.name
        ...
        if cidr is not None:
            ...
            resolved_veth_addr = str(hosts[1])
            resolved_vpeer_addr = str(hosts[0])
        ...
        try:
            ...
            # Configure host side
            await run(["ip", "addr", "add", f"{resolved_veth_addr}/{prefix_len}", "dev", veth], check=True)
            await run(["ip", "link", "set", veth, "up"], check=True)

            # Configure namespace side
            await run(["ip", "addr", "add", f"{resolved_vpeer_addr}/{prefix_len}", "dev", vpeer], check=True, preexec_fn=namespace.enter)
            ...
            yield NetworkInterfaces(name, veth, vpeer, resolved_veth_addr, resolved_vpeer_addr, prefix_len)
        finally:
